...
class _HebPlusPageState extends State<HebPlusPage> {
  ...
  Future<void> _loadCsv() async {
    try {
      final csvString = await DefaultAssetBundle.of(context).loadString('lib/assets/HEBplus.csv');
    ...
    setState(() {
      ...
      if (_selectedCategory == null || _selectedCategory == 'All') {
        ...
        if (cat == 'greens' || cat == 'grapes') {
          ...
          // Special case: include both oranges and tangerines, but exclude bell peppers
          _filteredPluList = searchFiltered.where((e) {
            final name = (e['name'] ?? '').toLowerCase();
            // Match orange or tangerine, but exclude if name contains 'pepper' or 'bell'
            if (name.contains('pepper') || name.contains('bell')) return false;
            return RegExp('\\b(orange|tangerine)s?\\b').hasMatch(name);
          }).toList();
        } else {
          ...
          _filteredPluList = searchFiltered.where((e) {
            final name = (e['name'] ?? '').toLowerCase();
            // match: does the name contain any form of the category word using word boundaries?
            return forms.any((form) => RegExp('\\b' + form + '\\b').hasMatch(name));
          }).toList();
        }
      }
    });
